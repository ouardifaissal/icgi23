...
if ( ! class_exists( 'The_Conference_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class The_Conference_WebFont_Loader {
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'the_conference_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'the_conference_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'the_conference_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'the_conference_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function delete_fonts_folder() {
			...
			delete_site_option( 'the_conference_local_font_files' );
...
if ( ! function_exists( 'the_conference_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function the_conference_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new The_Conference_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'the_conference_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function the_conference_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new The_Conference_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
/**
 * Create instance of The_Conference_WebFont_Loader class.
 */
function the_conference_webfont_loader_instance( $font_url = '' ) {
	return new The_Conference_WebFont_Loader( $font_url );
}
